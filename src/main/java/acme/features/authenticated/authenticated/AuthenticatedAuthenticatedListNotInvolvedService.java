...
@Service
public class AuthenticatedAuthenticatedListNotInvolvedService implements AbstractListService<Authenticated, Authenticated> {
	...
	public Collection<Authenticated> findMany(final Request<Authenticated> request) {
		...
		result = this.repository.findAuthenticatedNotInvolved(id);
